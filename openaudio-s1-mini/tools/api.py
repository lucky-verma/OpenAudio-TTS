...
def load_audio(reference_audio, sr):
    ...
    # Try to load with torchaudio, with fallback to soundfile library
    try:
        waveform, original_sr = torchaudio.load(reference_audio, backend=backend)
    except (ImportError, RuntimeError) as e:
        # If torchcodec is not available or other error, try soundfile directly
        if "torchcodec" in str(e).lower() or "TorchCodec" in str(e):
            logger.warning(f"TorchCodec not available, falling back to soundfile library: {e}")
            try:
                # If reference_audio is BytesIO, we need to handle it
                if isinstance(reference_audio, io.BytesIO):
                    reference_audio.seek(0)
                    data, original_sr = sf.read(reference_audio)
                    waveform = torch.from_numpy(data).unsqueeze(0)  # Add channel dimension
                else:
                    data, original_sr = sf.read(reference_audio)
                    waveform = torch.from_numpy(data).unsqueeze(0)
            except Exception as sf_error:
                logger.error(f"Failed to load audio with soundfile: {sf_error}")
                raise ImportError(
                    "Unable to load audio. Please install torchcodec or ensure soundfile backend is available. "
                    f"Original error: {e}, Soundfile error: {sf_error}"
                )
        else:
            raise
